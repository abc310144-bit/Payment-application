...
interface Props {
  ...
  /** work＝核銷作業頁可回壓；history＝檢視頁僅看歷史 */
  variant?: 'work' | 'history'
}

export function WriteoffHistoryPanel({ app, variant = 'work' }: Props) {
  const { role } = useRole()
  ...
  const historyOnly = variant === 'history'
  const hasAnyRecord = rows.some((row) => writebacksOf(row).length > 0)

  if (historyOnly && !hasAnyRecord && rows.length === 0) {
    return null
  }
  ...
  return (
    <div className={`writeoff-panel${historyOnly ? ' is-history' : ''}`}>
      {!historyOnly && (
        <p className="writeoff-summary">
          母單：{app.applicationNo}（未核銷餘額：
          {formatAmount(remaining, currency)}）
        </p>
      )}
      {historyOnly && (
        <h3 className="writeoff-history-title">核銷歷史</h3>
      )}
      {historyOnly && !hasAnyRecord && (
        <p className="writeoff-empty-inline">尚無核銷回壓紀錄。</p>
      )}
          ...
          <table className="writeoff-table">
            ...
            <tbody>
              ...
              {rows.map((row, idx) => {
                ...
                if (historyOnly && records.length === 0) return null
                return (
                  <Fragment key={row.id}>
                    {!historyOnly && (
                      <tr>
                    ...
                    {records.map((record) => (
                      <tr key={record.id} className="writeoff-child">
                        <td colSpan={3}>
                          {formatWritebackLabel(record, currency)}
                        </td>
                        <td>
                          <div className="writeoff-ops">
                            <button
